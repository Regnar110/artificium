  ...
  return (
    <div id='currently_online' className='flex flex-col gap-4 '>
        ...
        <div id='online_list' className='flex flex-col gap-3 opacity-50 '>
          {
            friends.map(el => <UserAvatarWithStatus key={el._id} user_avatar={user_avatar} user_data={{nickname:el.nickname, }} user_status={{with_dot:true, with_text:true,  status:el.isOnline === true ? "ONLINE":"OFFLINE"}}/>)
          }
        </div>
    </div>
  )
