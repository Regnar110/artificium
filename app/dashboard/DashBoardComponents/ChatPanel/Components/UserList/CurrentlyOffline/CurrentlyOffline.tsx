  ...
  return (
    <div id='currently_online' className='flex flex-col gap-4 '>
        ...
        <div id='online_list' className='flex flex-col gap-3 opacity-50 '>
          {
            friends.map(el => <UserAvatarWithStatus size='normal' key={el._id} user_avatar={user_avatar} user_data={el} show_nick={true} user_status={{with_dot:true, with_text:true,  status:el.isOnline === true ? "ONLINE":"OFFLINE"}} modal_action={true}/>)
          }
        </div>
    </div>
  )
