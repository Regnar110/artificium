...
const CUSTOM_TOAST_GROUP_ACTION = ({message, user, group_name}:Props) => {
  return (
    <section className='TOAST_group_action bg-[#060708] w-[280px] font-plus_jakarta_sans flex flex-col gap-2 justify-center items-start px-5 py-2 rounded-md shadow-sm shadow-[#B6F09C]'>
        ...
        <UserAvatarWithStatus size='normal' user_avatar={avatar} show_nick={true} user_status={{status:user?.isOnline ? "ONLINE" : "OFFLINE",with_dot:true, with_text:false}} modal_action={false} user_data={user} reveal_mail={false} message={message} hex_font_color='#fff'/>
    </section>
  )
}
